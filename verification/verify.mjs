...
const mutatedManifest = await fsp.readFile(path.join(mutation.outputRoot,'rendered','prod-rollback.yaml'),'utf8'); assert(mutatedManifest.includes('tenant-alpha-rps') && mutatedManifest.includes('520'), 'limit change not reflected in rendered manifest');
